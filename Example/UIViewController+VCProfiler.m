...
static void pmy_loadView(UIViewController *kvo_self, SEL _sel) {
    ...
    //把 origin_imp 定义成函数指针
    void (*func)(UIViewController *, SEL) = (void (*)(UIViewController *, SEL))origin_imp;

    VCLog(@"VC: %@ -loadView \t\tbegin  at CF time:\t%lf", kvo_self, CFAbsoluteTimeGetCurrent());
    //通过 函数指针 调用 IMP 
    func(kvo_self, _sel);
    VCLog(@"VC: %@ -loadView \t\tfinish at CF time:\t%lf", kvo_self, CFAbsoluteTimeGetCurrent());
}
